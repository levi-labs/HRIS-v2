...
import {
    TransferRequestForEmployee,
    TransferRequestForHrd,
} from '../types/transferRequest.type.js';
import { Validation } from '../validations/validation.js';
import {
    transferRequestForEmployeeSchema,
    transferRequestForHRDSchema,
} from '../validations/transferRequest.validation.js';
...
export class TransferRequestService {
    ...
    static async createFromEmployee(data: TransferRequestForEmployee): Promise<TransferRequest> {
        const validate = Validation.validate<TransferRequestForEmployee>(
            transferRequestForEmployeeSchema,
            data,
        );
    ...
    static async updateFromEmployee(
        id: number,
        data: TransferRequestForEmployee,
    ...
    static async createFromHRD(data: TransferRequestForHrd): Promise<TransferRequest> {
        const validate = Validation.validate<TransferRequestForHrd>(
            transferRequestForHRDSchema,
            data,
        );
        const count = await prisma.transferRequest.count({
    ...
    static async approve(id: number): Promise<TransferRequest> {
        const checkExistRequest = await prisma.transferRequest.findUnique({
            where: {
                id,
            },
        });
        ...
        if (checkExistRequest.status !== TransferRequestStatus.PENDING) {
            throw new ResponseError(
                409,
                'Transfer request cannot be approved because it is not pending',
            );
        }
    ...
    static async reject(id: number): Promise<TransferRequest> {
        ...
        if (checkExistRequest.status !== TransferRequestStatus.PENDING) {
            throw new ResponseError(
                409,
                'Transfer request cannot be rejected because it is not pending',
            );
        }
