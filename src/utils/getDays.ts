...
export const getTimeNow = (schedule:any ,type:string) => {
   ...
   if (type === "start") {
      const workTimeTolerance = new Date(workTimeInWIB + 7 * 60 * 60 * 1000 + schedule.late_tolerance * 60000);
      ...
      if(nowWIB.getHours() > 18){
         ...
         const workTimeTolerance = new Date(workTimeInWIB + 7 * 60 * 60 * 1000 + schedule.early_tolerance * 60000);
         return {
            time:currentTimeUTC,
            status:AttendanceStatus.PRESENT,
            nowWIB
